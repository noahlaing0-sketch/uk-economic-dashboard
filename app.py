...
try:
    ...
    st.error(
        "Couldn't load the economic data. The ONS or Bank of England servers may be "
...
if st.button("Generate explanation"):
    with st.spinner("Analysing the data..."):
        try:
            ...
            st.error(
                "Couldn't generate the explanation right now. The AI service may be "
                "unavailable or the API key may need checking. The data and charts "
...
if question:
    with st.spinner("Looking up the data..."):
        try:
            ...
            st.error(
                "Couldn't answer that right now. The AI service may be unavailable. "
                "The charts and data above are unaffected."
            )
            st.caption(f"Technical detail: {type(e).__name__}")